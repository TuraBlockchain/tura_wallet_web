...
var BRS = (function(BRS, $, undefined) {
    ...
    BRS.loadAsset = function(asset, refresh) {
        ...
        if (BRS.accountInfo.unconfirmedBalanceNQT == "0") {
            ...
            $("#your_burst_balance").html(BRS.formatAmount(BRS.accountInfo.unconfirmedBalanceNQT)*10000);
        ...
        BRS.sendRequest("getTrades+" + assetId, {
            ...
            if (response.trades && response.trades.length) {
                ...
                for (var i = 0; i < trades.length; i++) {
                    trades[i].priceNQT = new BigInteger(trades[i].priceNQT)*10000;
    ...
    $("#asset_order_modal").on("show.bs.modal", function(e) {
        ...
        try {
            ...
            // console.log(String($("#" + orderType + "_asset_price").val()) + ','+priceNQT)
        ...
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "burst": BRS.formatAmount(priceNQTPerWholeQNT)*10000
            });
        ...
        $("#asset_order_total").html(totalNXT + " Tura");
        $("#asset_order_fee_paid").html((BRS.formatAmount(feeNQT))*10000 + " Tura");
    ...
    BRS.forms.orderAssetComplete = function(response, data) {
        ...
        console.log( data.priceNQT.toString().escapeHTML())
        data.totalNQT = new BigInteger(BRS.calculateOrderTotalNQT(data.quantityQNT, data.priceNQT));

        var rowToAdd = "<tr class='tentative' data-transaction='" + String(response.transaction).escapeHTML() + "' data-quantity='" + data.quantityQNT.toString().escapeHTML() + "' data-price='" + data.priceNQT.toString().escapeHTML() + "'><td>You - <strong>Pending</strong></td><td>" + BRS.formatQuantity(data.quantityQNT, BRS.currentAsset.decimals) + "</td><td>" + BRS.formatOrderPricePerWholeQNT(data.priceNQT*10000, BRS.currentAsset.decimals)+ "</td><td>" + BRS.formatAmount(data.totalNQT) + "</td></tr>";
